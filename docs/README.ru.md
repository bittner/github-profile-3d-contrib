...
Каждое изображение содержит и светлую, и темную цветовую схему и переключается между ними автоматически в соответствии со схемой, которую сообщает браузер зрителя (`prefers-color-scheme`). Отдельного светлого или темного файла для выбора нет. Если нужно изображение с одной фиксированной схемой, используйте `SETTING_JSON` с файлом настроек без блока `darkMode`, как большинство файлов в `sample-settings/`.
